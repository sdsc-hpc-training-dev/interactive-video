window.addEventListener("load", async () => {
    ...
    const typeNode = document.getElementById("iv-type");
    if (typeNode && typeNode.hasAttribute("webinar")) {
        try {
    ...
    searchInput.onchange = () => {
        ...
        if (!fuse) {
            ...
            result.innerHTML = "";
            count.innerText = `${arr.length} result${
                arr.length > 1 ? "s" : ""
            ...
            for (const match of arr.slice(0, 100)) {
                ...
                const str = new Date(seconds * 1000)
                    .toISOString()
                    .substr(11, 8);
